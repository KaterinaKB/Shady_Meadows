...
class AdminMessagesPage:
    ...
    @allure.step("Check existence of the message from {author} with subject {subject}")
    ...
    @allure.step("Open the message with topic {subject}")
    def open_message(self, subject):
        self.subject_of_message.element_by(have.exact_text(subject)).click()

    @allure.step("Check content of the message from {author} with subject {subject}")
