...
class MessageForm:
    def __init__(self):
        ...
        self.subject_in_reply = browser.element(".col-sm-5").element(
            """//p[text()="We'll get back to you about"]/following::p[1]"""
        )
    ...
    @allure.step(
        "Send the confirmation of sending the message from author {name} with subject {subject}"
    )
    def check_reply_to_message(self, name, subject):
        self.thanks_for_message.should(
            have.exact_text(f"Thanks for getting in touch {name}!")
        )
