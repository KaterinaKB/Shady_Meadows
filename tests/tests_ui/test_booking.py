...
class TestsBooking:
    ...
    @allure.title(
        "Test for calculating the total cost of booking for {duration} nights"
    )
    ...
    def test_booking_creation(self, setup_browser, delete_data_after_test):
        ...
        # WHEN
        app.booking_form.create_booking(start_date, end_date, client)
    ...
    def test_confirmation_data_after_booking(
        self, setup_browser, delete_data_after_test):
        # GIVEN
        app.main_page.open()
        app.main_page.press_open_booking_button()
        with step("Generate booking dates"):
